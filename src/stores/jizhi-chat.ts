...
import type { HostJizhiChatMessage, HostJizhiStreamData, HostJizhiStreamEvent } from '@/lib/jizhi-chat';

export interface JizhiChatMessage extends HostJizhiChatMessage {}

type PendingMessagePair = {
  prompt: string;
  assistantMessageUUID: string;
  messages: JizhiChatMessage[];
};

type AppendPendingMessageParams = {
  sessionId: string;
  prompt: string;
  assistantMessageUUID: string;
  model?: string;
...
type AppendPendingAssistantParams = {
  sessionId: string;
  assistantMessageUUID: string;
  model?: string;
  modelName?: string;
...
export const useJizhiChatStore = create<JizhiChatState>((set) => ({
  ...
  appendPendingMessagePair: ({ sessionId, prompt, assistantMessageUUID, model, modelName }) => set((state) => {
  ...
  appendPendingAssistant: ({ sessionId, assistantMessageUUID, model, modelName, placeholderText }) => set((state) => {
    const now = new Date().toISOString();
  ...
  markPendingPairError: (sessionId, assistantMessageUUID, errorMessage) => set((state) => {
  ...
  applyStreamEvent: ({ sessionId, messageUUID, event, data, errorMessage }) => set((state) => {
    const currentPairs = state.pendingMessagesBySession[sessionId] ?? [];
    if (currentPairs.length === 0) return state;
