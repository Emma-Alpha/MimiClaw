...
const contentStyles = createStaticStyles(({ css, cssVar }) => ({
  codeWrapper: css`
    position: relative;
    overflow-x: auto;
...
const ContentViewer = memo<ContentViewerProps>(({ selectedFile, contentMap, skillContent }) => {
  if (selectedFile === 'SKILL.md') {
    const displayContent = skillContent;
    if (!displayContent) {
      return (
        <div className={contentStyles.docWrapper}>
  ...
  return (
    <div className={contentStyles.codeWrapper}>
      <CopyButton
        content={content}
