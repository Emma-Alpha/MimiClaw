...
# Post-installation script for MimiClaw on Linux
...
# Create symbolic link for MimiClaw app binary
if [ -x /opt/MimiClaw/mimiclaw ]; then
    ln -sf /opt/MimiClaw/mimiclaw /usr/local/bin/mimiclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/MimiClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/MimiClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/MimiClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/MimiClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/mimiclaw'
...
echo "MimiClaw has been installed successfully."
