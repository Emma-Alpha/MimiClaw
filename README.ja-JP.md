...
メインウィンドウのサイドバーはシナリオ別フォルダ（チャット / CLI / 極智 / 小九 / 音声）で会話を整理し、折りたたみ後もタイトルバーのボタンから即座に再表示できます。
...
フローティングペットの音声入力は、マイク音声を Volcengine のリアルタイム ASR にストリーミングし、途中経過の文字起こしを表示しながら、録音終了後に認識テキストを Mini Chat へ直接送ります。利用前に **設定 → 通用 → Volcengine ASR** で `App ID`、`Cluster`、`Access Token` を設定してください。
さらに、フローティングペットの右クリックメニューから専用の **音声会話** ウィンドウを開き、Volcengine のリアルタイム音声機能で全二重の常時マイク会話を行えるようになりました。安定した発話は読み取り専用の `Voice Chat` 履歴へ同期されます。利用前に **設定 → 通用 → 音声会話** で `APP ID` と `Access Token` を設定してください。Volcengine ASR を設定済みなら、既存の `Access Token` をそのまま再利用できます。
同じ右クリックメニューには **コードアシスタント** も追加され、Mini Chat を持続的な `CLI` モードで直接開けます。手動で通常チャットへ戻すかミニウィンドウを閉じるまで、その後のメッセージも Claude Code ワークフローに送り続けます。
Mini Chat / CLI のヘッダーは、実行中タスクがある間は現在の Agent / スレッドを動的インジケーター付きで表示します。
macOS のメニューバーアイコンを左クリックすると、Gateway/CLI の実行状況を双環メーターとスレッド一覧で表示する専用モニターパネルを開けます。
トレイメニューには実行中スレッド（Gateway + CLI）が最大 2 件まで表示され、項目をクリックすると対応セッションへ直接移動できます。
...
极智 には Tencent 公式の個人 WeChat チャンネルプラグインも同梱されており、Channels ページからアプリ内 QR フローで直接 WeChat を連携できます。
...
### 🚀 自動起動設定
**設定 → 通用** から **システム起動時に自動起動** を有効化すると、ログイン後に 极智 が自動的に起動します。
