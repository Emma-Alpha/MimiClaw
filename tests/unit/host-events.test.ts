...
describe('host-events', () => {
  ...
  it('subscribes through IPC for code-agent events', async () => {
    const onMock = vi.mocked(window.electron.ipcRenderer.on);
    const offMock = vi.mocked(window.electron.ipcRenderer.off);
    const captured: Array<(...args: unknown[]) => void> = [];
    onMock.mockImplementation((_, cb: (...args: unknown[]) => void) => {
    ...
    const { subscribeHostEvent } = await import('@/lib/host-events');
    const handler = vi.fn();
    const unsubscribe = subscribeHostEvent('code-agent:run-completed', handler);

    expect(onMock).toHaveBeenCalledWith('code-agent:run-completed', expect.any(Function));

    captured[0]({ result: { runId: 'abc' } });
    expect(handler).toHaveBeenCalledWith({ result: { runId: 'abc' } });

    unsubscribe();
    expect(offMock).toHaveBeenCalledWith('code-agent:run-completed', expect.any(Function));
  });
  ...
  it('uses SSE fallback only when explicitly enabled', async () => {
    window.localStorage.setItem('mimiclaw:allow-sse-fallback', '1');
    const { subscribeHostEvent } = await import('@/lib/host-events');
    const handler = vi.fn();
