 ...
 * Used by MimiClaw to seed the provider store when it's empty but providers are
...
/**
 * Write the MimiClaw gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    // Packaged MimiClaw loads the renderer from file://, so the gateway must allow
 ...
 * which means conversations disappear after roughly one day.  MimiClaw sets
 ...
 * OpenClaw/MimiClaw versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // When MimiClaw-managed moonshot provider exists, prefer centralized key
    ...
    // for MimiClaw to properly integrate with its updated tool system.
    ...
    // but MimiClaw historically stored them only under `channels.<type>.accounts.default`.
