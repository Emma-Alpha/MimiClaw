...
interface SyncProxyOptions {
   ...
   * currently disabled in MimiClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync MimiClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because MimiClaw proxy is disabled and preserve mode is enabled');
      return;
    }
