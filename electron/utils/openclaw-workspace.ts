...
const MIMICLAW_BEGIN = '<!-- mimiclaw:begin -->';
const MIMICLAW_END = '<!-- mimiclaw:end -->';
...
/**
 * Merge a MimiClaw context section into an existing file's content.
 * If markers already exist, replaces the section in-place.
 * Otherwise appends it at the end.
 */
export function mergeMimiClawSection(existing: string, section: string): string {
  const wrapped = `${MIMICLAW_BEGIN}\n${section.trim()}\n${MIMICLAW_END}`;
  const beginIdx = existing.indexOf(MIMICLAW_BEGIN);
  const endIdx = existing.indexOf(MIMICLAW_END);
  if (beginIdx !== -1 && endIdx !== -1) {
    return existing.slice(0, beginIdx) + wrapped + existing.slice(endIdx + MIMICLAW_END.length);
  }
...
/**
 * Detect and remove bootstrap .md files that contain only MimiClaw markers
...
export async function repairMimiClawOnlyBootstrapFiles(): Promise<void> {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of entries) {
      ...
      const beginIdx = content.indexOf(MIMICLAW_BEGIN);
      const endIdx = content.indexOf(MIMICLAW_END);
      if (beginIdx === -1 || endIdx === -1) continue;

      const before = content.slice(0, beginIdx).trim();
      const after = content.slice(endIdx + MIMICLAW_END.length).trim();
      if (before === '' && after === '') {
        try {
          await unlink(filePath);
          logger.info(`Removed MimiClaw-only bootstrap file for re-seeding: ${file} (${workspaceDir})`);
        } catch {
          logger.warn(`Failed to remove MimiClaw-only bootstrap file: ${filePath}`);
        }
      }
    }
  }
...
/**
 * Merge MimiClaw context snippets into workspace bootstrap files that
 * already exist on disk.  Returns the number of target files that were
 * skipped because they don't exist yet.
 */
async function mergeMimiClawContextOnce(): Promise<number> {
  const contextDir = join(getResourcesDir(), 'context');
  if (!(await fileExists(contextDir))) {
    logger.debug('MimiClaw context directory not found, skipping context merge');
    return 0;
  }

  let files: string[];
  try {
    files = (await readdir(contextDir)).filter((f) => f.endsWith('.mimiclaw.md'));
  } catch {
    return 0;
  }
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of files) {
      const targetName = file.replace('.mimiclaw.md', '.md');
      ...
      const merged = mergeMimiClawSection(existing, section);
      if (merged !== existing) {
        await writeFile(targetPath, merged, 'utf-8');
        logger.info(`Merged MimiClaw context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
/**
 * Ensure MimiClaw context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureMimiClawContext(): Promise<void> {
  let skipped = await mergeMimiClawContextOnce();
  if (skipped === 0) return;

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    await new Promise((r) => setTimeout(r, RETRY_INTERVAL_MS));
    skipped = await mergeMimiClawContextOnce();
    if (skipped === 0) {
      logger.info(`MimiClaw context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`MimiClaw context merge: ${skipped} file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`MimiClaw context merge: ${skipped} file(s) still missing after ${MAX_RETRIES} retries`);
}
