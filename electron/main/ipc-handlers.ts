...
import {
	createVolcengineAsrSession,
	type VolcengineAsrSession,
} from "../services/volcengine-asr";
import {
	createVolcengineRealtimeVoiceSession,
	type VolcengineRealtimeVoiceSession,
} from "../services/volcengine-realtime-voice";
...
function isAuxiliaryWindowUrl(url: string): boolean {
	return (
...
function getMainAppWindow(): BrowserWindow | undefined {
	return BrowserWindow.getAllWindows().find((window) => {
		...
		return !isAuxiliaryWindowUrl(window.webContents.getURL());
	});
}
...
function buildPetCompanionSeed(
	storedSeed: string,
	machineId: string,
): string {
...
async function recordPetCompanionUsage(
	...
	const updated = applyPetCompanionGrowth(companion, action);
	await setSetting("petCompanion", updated);
...
function registerPetHandlers(): void {
	...
	ipcMain.handle(
		...
		async (
			...
			const activity =
				typeof payload === "string" ? payload : payload?.activity;
			setPetInputActivity(
				activity === "recording" || activity === "transcribing"
					? activity
					: "idle",
			);
	...
	ipcMain.handle(
		"pet:setUiActivity",
		async (_, payload?: { activity?: PetUiActivity } | PetUiActivity) => {
			const activity =
				typeof payload === "string" ? payload : payload?.activity;
			setPetUiActivity(
				activity === "working" || activity === "listening" ? activity : "idle",
			);
			return { success: true, state: getPetRuntimeState() };
		},
	);
	...
	ipcMain.handle(
		...
		async (_event, payload?: { action?: "start" | "cancel" | "confirm" }) => {
			const action =
	...
	ipcMain.handle(
		...
		async (
			_event,
			payload?: { sessionId?: string; audioChunk?: Uint8Array },
		) => {
			...
			const chunk =
				payload?.audioChunk instanceof Uint8Array
					? payload.audioChunk
	...
	ipcMain.handle(
		...
		async (_event, payload?: { visible?: boolean } | boolean) => {
			const visible =
	...
	ipcMain.handle(
		"pet:showContextMenu",
		async (event, payload?: { x?: number; y?: number; language?: string }) => {
					...
					: []),
				{
					...
					click: () => {
						...
						if (mainWin) {
							...
							mainWin.show();
							mainWin.focus();
							mainWin.webContents.send("navigate", "/");
						}
					},
				},
				{ type: "separator" },
	...
	ipcMain.handle("pet:move", (event, payload?: { x?: number; y?: number }) => {
		...
		if (win && !win.isDestroyed()) {
			const x =
				typeof payload?.x === "number" ? Math.round(payload.x) : undefined;
			const y =
				typeof payload?.y === "number" ? Math.round(payload.y) : undefined;
	...
	ipcMain.handle("pet:openCodeAssistant", async (_event) => {
		await recordPetCompanionUsage("code_assistant");
		const mainWin = getMainAppWindow();
		if (mainWin) {
			if (mainWin.isMinimized()) mainWin.restore();
	...
	function emitVoiceRealtimeEvent(
		sessionId: string,
		payload: Record<string, unknown>,
	): void {
	...
	ipcMain.handle(
		...
		async (
			_event,
			payload?: { state?: "idle" | "connecting" | "connected" },
		) => {
			const nextState =
	...
	ipcMain.handle("voice:sessionStart", async () => {
		...
		const realtimeSession = await createVolcengineRealtimeVoiceSession(
			(event) => {
				emitVoiceRealtimeEvent(
					realtimeSessionId,
					event as Record<string, unknown>,
				);
				if (
					event.type === "voice.connection.error" ||
					(event.type === "voice.connection.status" &&
						event.status === "closed")
				) {
	...
	ipcMain.handle(
		...
		async (
			_event,
			payload?: { realtimeSessionId?: string; audioChunk?: Uint8Array },
		) => {
	...
	ipcMain.handle(
		"voice:commitTurn",
		async (_event, payload?: { realtimeSessionId?: string }) => {
			const realtimeSessionId = payload?.realtimeSessionId?.trim();
			if (!realtimeSessionId) {
				throw new Error("realtimeSessionId is required");
			}
					...
					case "provider": {
						...
						if (request.action === "validateKey") {
							...
							const resolvedProtocol =
								options?.apiProtocol || provider?.apiProtocol;
...
function registerOpenClawHandlers(gatewayManager: GatewayManager): void {
	...
	const forceRestartChannels = new Set([
		"dingtalk",
		"wecom",
		"whatsapp",
		"feishu",
	...
	const scheduleGatewayChannelSaveRefresh = (
		channelType: string,
		reason: string,
	): void => {
		if (gatewayManager.getStatus().state === "stopped") {
